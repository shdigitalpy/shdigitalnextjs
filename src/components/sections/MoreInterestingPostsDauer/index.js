...
const itemsLeft = {
    ...
    pointsData: [
        [
            ...
            { link: true, url: '/seo/seo-analyse', data: 'SEO-Analyse'},
            { link: false, data: 'in der Planungsphase vor.'}
        ],
        
        [
            { link: false, data: 'Welche'},
            { link: true, url: '/seo/seo-tools', data: 'SEO-Tools'},
                        ...
                        { itemsRight?.pointsData.map((item, index) => (
                            <FramerMotionAnimation
                                ...
                                <div className="item-description">
                                    { item.map((node, i) => (
                                        <React.Fragment key={i}>
                                            { node.link ? <Link target="__blank" href={node.url} title={node.data} className="text-link">{node.data}</Link>
                                                : <span>{node.data}</span>}
                                        </React.Fragment>
                                    ))}
                                </div>
